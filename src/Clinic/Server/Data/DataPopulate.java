...
public class DataPopulate {
    ...
    private static  void popPrescriptions() {
        ArrayList<Prescription>  list = new ArrayList<>();

        //list.add(new Prescription("Drug", LocalDate.now(), "This is a drug that cures being sick", "Take every  ten minutes", ))
    }

    private static void popAppointment() {

        ArrayList<Appointment> list = new ArrayList<>();
     //   list.add(new Appointment(new DataReader().readPatients().get(0), new DataReader().readDoctors().get(0), LocalDate.of(2019, 12, 2), new Diagnosis(), new Prescription()));
      //  list.add(new Appointment(new DataReader().readPatients().get(1), new DataReader().readDoctors().get(1), LocalDate.of(2019, 12, 2), );
